		...
		throws IOException
	{
		...
		while(li.hasNext())
		{
			...
			else if(isClauseWord(next.toString()))
			{
				...
				{
					if(!value.trim().equals("")) qs.addSelectList(new QueryTokens.DefaultExpression(value.trim()));
					break;
				}
			}
			else
...
//				System.out.println("join");
				...
				if(t!=null) tables.put(stripQuote(t.getReference()),t);
				t=null;
...
//				System.out.println("table or alias");
		...
		throws IOException
	{
		...
		for(int surrounds = 0; li.hasNext();)
		{
			...
			else
			{
				...
				{
					char last = value.charAt(value.length()-1);
					if(Character.isLetter(last)) value = value + SQLFormatter.SPACE;
				}
				value = value + next.toString();
	...
	private static void doEnsureReferences(QuerySpecification qs)
		throws IOException
	{
		Hashtable tables = new Hashtable();
		...
		for(int i=0; i<fromClause.length; i++)
		{
			...
			{
				tables.put(stripQuote(((QueryTokens.Join)token).getPrimary().getTable().getReference()),((QueryTokens.Join)token).getPrimary().getTable());
				tables.put(stripQuote(((QueryTokens.Join)token).getForeign().getTable().getReference()),((QueryTokens.Join)token).getForeign().getTable());
			}
			else
			{
				...
				if(stream.ttype == StreamTokenizer.TT_EOF)
				{
					...
					if(dot!=-1)
					{
						...
						if(tables.containsKey(owner))
						{
							...
							if(alias!=null) ((QueryTokens.Column)selectList[i]).setAlias(alias);
						}
					}
					break;
				}
