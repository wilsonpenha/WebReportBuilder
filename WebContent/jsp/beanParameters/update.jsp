...
<input type="hidden" name="objIds" value="<%=beanPage.getParametersId()%>">
    ...
    <td>
          <table width="100%" border="0" cellPadding="0" cellSpacing="0">
            <tr>              <td width="32%" class="dataLabel">*Parameter Name : </td>              <td width="66%" class="inputLocation">                <input type="hidden" name="parametersId" value="<%=beanPage.getParametersId()%>">                <input type="hidden" name="reportId" value="<%=beanPage.getReportId()%>">                <input type="text" class="dataInput" name="parameterName" tabIndex="3" maxlength="30" size="30" value="<%=beanPage.getParameterName()%>">              </td>            </tr>            <tr>              <td width="32%" class="dataLabel">*Class Type : </td>              <td width="66%" class="inputLocation">                <%
              ...
              </td>            </tr>            <tr>              <td width="32%" class="dataLabel">Parameter Description : </td>              <td width="66%" class="inputLocation">                <input type="text" class="dataInput" name="parameterDescription" tabIndex="5" maxlength="50" size="50" value="<%=beanPage.getParameterDescription()%>">              </td>            </tr>            <tr>
          ...
          <table width="100%" border="0" cellPadding="0" cellSpacing="0">
            <tr>
              ...
              <td width="66%" class="inputLocation">
                <input type="hidden" name="sqlStr" value="<%=beanPage.getSqlStr()%>">
                <textarea class="dataInput" name="tableInput" tabIndex="3" rows="4" cols="55"><%if (beanPage.getSqlStr().equals("")){%><%=beanPage.getTableInput()%><%}else{%><%=beanPage.getSqlStr()%><%}%></textarea>
                <input type="button" value=" Load fields "  style="width: 69px;" name="btnSave" class="stdButton" onMouseOver="javascript:mensagem('Click here to load fields.');" onClick="javascript:document.forms[0].sqlStr.value=document.forms[0].tableInput.value;setAction(document.forms[0],'<%=BeanObject.ACTION_UPDATE_EDIT%>');document.forms[0].submit();" >
              </td>
            </tr>
            <tr>
	      ...
	      <%  reports.bean.BeanParameters beanParams = new reports.bean.BeanParameters();
	      	  ...
	      	  beanParams.setSqlStr(beanPage.getSqlStr());
